        ...
        <div>
            <div className="p-6 border-b border-gray-200/50">
                <div className="flex items-center justify-between">
                    <div>
                        <h3 className="text-lg font-semibold text-gray-900">Email Senders</h3>
                        ...
                        <div key={sender.id} className="p-6 hover:bg-gray-50/50 transition">
                            <div className="flex items-start justify-between">
                                <div className="flex items-start space-x-4">
                                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${sender.type === "gmail" ? "bg-red-100" : sender.type === "outlook" ? "bg-blue-100" : "bg-green-100"
                                        }`}>
                                        <Mail className={`w-5 h-5 ${sender.type === "gmail" ? "text-red-600" : sender.type === "outlook" ? "text-blue-600" : "text-green-600"
                                            }`} />
                                    </div>
                                    <div>
                                        <div className="flex items-center space-x-3">
                                            <h4 className="font-medium text-gray-900">{sender.email}</h4>
                                            <span className={`px-2 py-1 text-xs rounded-full ${getStatusColor(sender.isVerified ? "verified" : "pending")}`}>
                                                {sender.isVerified ? "Verified" : "Pending"}
                                            </span>
                                        </div>
                                        <p className="text-sm text-gray-600 mt-1">
                                            {getSenderIcon(sender.type)} • {sender.type === "smtp" ? `${sender.smtpHost}:${sender.smtpPort}` : "OAuth2"}
                                        </p>
                                    </div>
                                </div>
