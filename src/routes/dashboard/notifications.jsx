      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 animate-in fade-in slide-in-from-top-4 duration-700">
        <div className="flex flex-col">
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight flex items-center">
            {t('common.notifications', 'Notifications')} <span className="ml-2">Center</span>
          </h1>
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <aside className="lg:w-80 shrink-0 space-y-6 lg:sticky lg:top-24">
          <div className="bg-white/40  border border-slate-200/50 p-2.5 rounded-xl shadow-sm shadow-slate-900/5">
            <div className="mb-4 mt-2 px-3">
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full h-10 pl-11 pr-4 bg-white/60 border border-slate-200/60 rounded-md text-xs font-bold placeholder:text-slate-400 focus:bg-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/10 outline-none transition-all shadow-inner"
                />
              </div>
            </div>
            ...
            {[
              ...
              <button
                ...
                className={`w-full flex items-center p-3.5 rounded-lg transition-all duration-300 group mb-1 ${
                  activeTab === tab.id
              ...
              >
                ...
                <div className="ltr:text-left rtl:text-right flex-1">
                  <p
                    className={`font-black tracking-tight text-xs uppercase ${activeTab === tab.id ? 'text-slate-900' : 'text-slate-500'}`}
                  >
                    {tab.label}
                  </p>
                </div>
                {tab.badge > 0 && (
                  <span
                    className={`text-[10px] font-black tracking-widest px-2 py-0.5 rounded-lg ${activeTab === tab.id ? 'bg-orange-100 text-orange-600' : 'bg-slate-100 text-slate-400'}`}
                  >
                    {tab.badge}
                  </span>
                )}
            ...
            {[
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 p-3.5 rounded-lg transition-all duration-300 group ${
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full ${filterCategory === cat.id ? 'bg-slate-800' : 'bg-slate-300 group-hover:bg-slate-400'}`}
                />
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 p-4 rounded-xl border-2 border-slate-200/60 bg-white/40 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 hover:text-orange-600 hover:border-orange-500 hover:bg-white transition-all disabled:opacity-50 shadow-sm shadow-slate-900/5"
          >
          ...
          <div className="bg-white/40  border border-slate-200/50 rounded-2xl shadow-sm shadow-slate-900/5 min-h-[500px] overflow-hidden p-6 md:p-8">
            {isLoading ? (
              ...
              <div className="py-32 px-6 text-center flex flex-col items-center">
                <div className="w-24 h-24 bg-white/60 text-slate-300 rounded-2xl shadow-sm shadow-slate-900/5 flex items-center justify-center mb-8 border border-slate-100">
                ...
                {(activeTab !== 'all' || filterCategory !== 'all' || searchQuery) && (
                  <button
                    ...
                    className="mt-8 px-8 py-4 bg-white border border-slate-200 text-orange-600 rounded-lg text-[10px] font-black uppercase tracking-widest hover:border-orange-200 hover:bg-orange-50 transition-colors shadow-sm"
                  >
                    Clear All Filters
                  </button>
                )}
                  ...
                  {notifications.map((notification) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`group relative p-6 md:p-8 rounded-xl border transition-all duration-300 flex flex-col sm:flex-row gap-6 ${
                          !notification.read
                            ? `bg-white border-${color}-200 shadow-sm shadow-${color}-900/5 ring-1 ring-${color}-100`
                            : 'bg-white/60 border-slate-200/60 hover:bg-white hover:border-slate-300/60 shadow-md shadow-slate-900/5'
                        }`}
                      >
                        {!notification.read && (
                          <div
                            className={`absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-12 bg-${color}-500 rounded-r-lg`}
                          />
                        )}

                        <div className="flex gap-5 flex-1 items-start">
                          <div
                            className={`w-14 h-14 rounded-lg shrink-0 flex items-center justify-center bg-${color}-50 text-${color}-600 ring-1 ring-${color}-100 shadow-inner`}
                          >
                          ...
                          <div className="flex-1 mt-1">
                            <div className="flex flex-wrap items-center gap-3 mb-2">
                              <h4
                                className={`text-lg font-black tracking-tight ${!notification.read ? 'text-slate-900' : 'text-slate-700'}`}
                              >
                                {notification.title}
                              </h4>
                              {!notification.read && (
                                <span
                                  className={`px-2.5 py-1 rounded-[0.5rem] bg-${color}-100 text-${color}-700 text-[9px] font-black uppercase tracking-widest leading-none`}
                                >
                                  New
                                </span>
                              )}
                        ...
                        <div className="flex sm:flex-col items-center justify-start sm:justify-center gap-2 border-t sm:border-t-0 border-slate-100 pt-5 sm:pt-0 shrink-0">
                          {!notification.read && (
                            <button
                              ...
                              className="px-5 py-2.5 sm:w-32 bg-slate-50 border border-slate-200 rounded-md text-[10px] font-black uppercase tracking-widest text-slate-600 hover:border-orange-500 hover:text-orange-600 hover:bg-white transition-all shadow-xs"
                            >
                              Mark Read
                            </button>
                          )}
                          <button
                            ...
                            className="px-5 py-2.5 sm:w-32 bg-slate-50 border border-slate-200 rounded-md text-[10px] font-black uppercase tracking-widest text-slate-400 hover:border-orange-500 hover:text-orange-600 hover:bg-orange-50 transition-all shadow-xs"
                          >
                            Delete
                          </button>
                        </div>
