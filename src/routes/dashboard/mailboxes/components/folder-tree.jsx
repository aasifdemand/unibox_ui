...
const FolderTree = ({
  ...
  const getFolderIcon = (name) => {
    ...
    if (n.includes("spam") || n.includes("junk")) return <AlertTriangle className="w-4 h-4 text-amber-500" />;
    if (n.includes("trash") || n.includes("bin") || n.includes("deleted")) return <Trash2 className="w-4 h-4 text-red-500" />;
    if (n.includes("archive")) return <Archive className="w-4 h-4" />;
    if (n.includes("star") || n.includes("important")) return <Star className="w-4 h-4 text-amber-500 fill-amber-500" />;
  ...
  const renderFolder = (folder, depth = 0) => {
    ...
    return (
      <div key={folder.id}>
        ...
        >
          <span className={`w-5 h-5 mr-3 shrink-0 flex items-center justify-center transition-colors ${isSelected ? 'text-white' : 'text-slate-400 group-hover:text-blue-500'}`}>
            {getFolderIcon(folder.name)}
          </span>
          <span className={`text-sm truncate flex-1 text-left ${isSelected ? 'font-bold' : 'font-semibold'}`}>
            {folder.name}
          </span>
          {folder.unreadCount > 0 && (
            <span
              className={`text-[10px] font-bold px-2 py-0.5 rounded-lg ml-2 shadow-xs border ${isSelected
