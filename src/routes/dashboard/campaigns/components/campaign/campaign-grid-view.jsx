    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {campaigns.map((campaign) => {
        ...
        return (
          <div
            key={campaign.id}
            className={`group relative overflow-hidden premium-card bg-white border-slate-200/60 transition-all duration-500 hover:shadow-2xl hover:shadow-indigo-500/8 ${isSelected ? "border-indigo-500 ring-4 ring-indigo-500/5" : ""
              }`}
          >
            {/* Status Indicator Bar */}
            <div
              className={`absolute top-0 left-0 w-full h-1 ${campaign.status === "running"
                ? "bg-emerald-500"
            ...
            <div className="p-7 relative z-10">
              ...
              <div className="bg-slate-50/50 rounded-3xl p-5 border border-slate-100/50 mb-8 group/progress hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${campaign.status === "running" ? "bg-emerald-500 shadow-lg shadow-emerald-500/50" : "bg-indigo-500"}`}></div>
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">
                      Progress
                    </span>
                  </div>
                  <span className="text-lg font-black text-slate-800 tracking-tighter tabular-nums">
                    {progress}<span className="text-[10px] text-slate-400 ml-0.5">%</span>
                  </span>
                </div>
                <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200/20 group-hover/progress:h-4 transition-all duration-500">
                  <motion.div
                    ...
                    className={`h-full relative transition-all duration-300 ${campaign.status === "running"
              ...
              <div className="grid grid-cols-2 gap-3 mb-8">
                {[
                  { label: "Recipients", value: campaign.totalRecipients, icon: Target, rate: null },
                  ...
                  <div key={idx} className="bg-white rounded-2xl p-4 border border-slate-100 group/stat hover:border-indigo-100 hover:shadow-lg hover:shadow-indigo-500/5 transition-all duration-300">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-6 h-6 rounded-lg bg-slate-50 flex items-center justify-center group-hover/stat:bg-indigo-50 transition-colors">
                        <stat.icon className={`w-3 h-3 ${stat.rate ? "text-indigo-500" : "text-slate-400"}`} />
                      </div>
                      <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest leading-none">
                        {stat.label}
                      </span>
                    </div>
                    <p className={`text-sm font-black tracking-tight tabular-nums ${stat.rate ? "text-indigo-600" : "text-slate-800"}`}>
                      {stat.value?.toLocaleString() || "0"}
                      {stat.rate && stat.value !== "-" && <span className="ml-0.5 text-[10px] opacity-70">%</span>}
                    </p>
                  </div>
                ))}
              </div>
