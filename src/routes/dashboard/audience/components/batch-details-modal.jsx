    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className="relative w-full max-w-5xl max-h-[90vh] bg-white rounded-[32px] shadow-sm overflow-hidden flex flex-col"
        >
          {/* Header */}
          <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            ...
            <button
              onClick={onClose}
              className="w-10 h-10 rounded-md hover:bg-slate-100 flex items-center justify-center text-slate-400 transition-colors"
            >
              <X className="w-6 h-6" />
            </button>
          </div>
            ...
            {isLoading ? (
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    {
                      label: t('audience.valid'),
                      value: verification?.valid || batch?.verification?.valid || 0,
                      color: 'emerald',
                    },
                    {
                      label: t('audience.risky'),
                      value: verification?.risky || batch?.verification?.risky || 0,
                      color: 'amber',
                    },
                    {
                      label: t('audience.invalid'),
                      value: verification?.invalid || batch?.verification?.invalid || 0,
                      color: 'rose',
                    },
                    {
                      label: t('audience.unverified'),
                      value: verification?.unverified || batch?.verification?.unverified || 0,
                      color: 'slate',
                    },
                  ].map((stat, i) => (
                    <div
                ...
                <div className="rounded-lg border border-slate-200/60 overflow-hidden">
                  <table className="w-full text-left text-sm border-separate border-spacing-0">
                    <thead className="bg-slate-50/80">
                      <tr>
                        <th className="px-6 py-4 text-[10px] font-bold text-slate-900 uppercase tracking-widest">
                          Email
                        </th>
                        <th className="px-6 py-4 text-[10px] font-bold text-slate-900 uppercase tracking-widest">
                          Name
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-10 h-10 rounded-md flex items-center justify-center text-slate-400 hover:bg-white hover:shadow-sm border border-transparent hover:border-slate-200 transition-all disabled:opacity-20"
                >
