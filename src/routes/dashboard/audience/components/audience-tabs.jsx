      ...
      <div className="bg-white/80 border-b border-slate-200/50 px-8">
        <nav className="flex items-center gap-10">
          <button
            ...
            className={`relative py-6 font-extrabold text-[10px] uppercase tracking-[0.2em] transition-all duration-300 ${activeTab === 'contacts' ? 'text-blue-600' : 'text-slate-400 hover:text-slate-800'
              }`}
          >
            <div className="flex items-center gap-2">
              <Users className="w-4 h-4" />
              {t('audience.contact_lists_tab')}
            </div>
            {activeTab === 'contacts' && (
              <div className="absolute bottom-0 ltr:left-0 ltr:right-0 rtl:left-0 ltr:right-0 rtl:left-0 h-1 bg-blue-600 rounded-t-full shadow-[0_-4px_12px_rgba(37,99,235,0.4)]"></div>
            )}
          </button>

          <button
            onClick={() => setActiveTab('senders')}
            className={`relative py-6 font-extrabold text-[10px] uppercase tracking-[0.2em] transition-all duration-300 ${activeTab === 'senders' ? 'text-blue-600' : 'text-slate-400 hover:text-slate-800'
              }`}
          >
            <div className="flex items-center gap-2">
              <Mail className="w-4 h-4" />
              {t('audience.email_senders_tab')}
            </div>
            {activeTab === 'senders' && (
              <div className="absolute bottom-0 ltr:left-0 ltr:right-0 rtl:left-0 ltr:right-0 rtl:left-0 h-1 bg-blue-600 rounded-t-full shadow-[0_-4px_12px_rgba(37,99,235,0.4)]"></div>
            )}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex-1 max-w-xl group">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder={t('audience.search_lists_placeholder')}
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full ltr:pl-12 ltr:pr-12 rtl:pl-12 ltr:pr-4 rtl:pl-4 py-3 bg-white border border-slate-200/60 rounded-2xl text-sm font-bold placeholder:text-slate-400 focus:bg-white focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500/50 outline-none transition-all"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <Filter className="absolute ltr:left-3 ltr:right-3 rtl:left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" />
                  <select
                    value={filterStatus}
                    onChange={(e) => setFilterStatus(e.target.value)}
                    className="ltr:pl-10 ltr:pr-10 rtl:pl-10 ltr:pr-4 rtl:pl-4 py-2.5 bg-white border border-slate-200/60 rounded-xl text-[10px] font-extrabold uppercase tracking-widest focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500/50 outline-none transition-all"
                  >
                    <option value="all">{t('audience.all_status')}</option>
                    <option value="uploaded">{t('audience.uploaded')}</option>
                    <option value="processing">{t('audience.processing')}</option>
                    <option value="completed">{t('audience.completed')}</option>
                    <option value="failed">{t('audience.failed')}</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            <AnimatePresence mode="wait">
              {isLoadingSenders ? (
                ...
                >
                  ...
                  <h3 className="text-xl font-extrabold text-slate-800 tracking-tight mb-2">
                    {t('audience.no_senders_title')}
                  </h3>
                  <p className="text-sm font-medium text-slate-400 max-w-xs mb-8">
                    {t('audience.no_senders_description')}
                  </p>
                  <button
                    onClick={() => setShowSenderModal(true)}
                    className="btn-primary py-3 px-8 flex items-center gap-3"
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-6 mb-8">
                        ...
                        <div className="flex flex-col">
                          ...
                          >
                            {sender.isVerified ? t('audience.active') : t('audience.pending')}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between pt-6 border-t border-slate-100/60">
                        <div className="flex gap-2">
                          <button
                            onClick={() => openSenderDetails(sender)}
