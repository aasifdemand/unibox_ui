...
export const useAnalyticsData = () => {
  ...
  } = useTimelineData(timeRange === '7' ? 'week' : timeRange === '30' ? 'month' : 'year');
  const {
    data: topCampaigns,
    isLoading: topCampaignsLoading,
  ...
  const timelineData = useMemo(() => {
    ...
    const items = Array.isArray(timeline) ? timeline : timeline?.timeline || timeline?.data || [];
    ...
    return items.map((item) => {
      ...
      const sent = Number(
        item.sent ?? item.emails_sent ?? item.sent_count ?? item.count ?? item.total_sent ?? 0,
      );
      const opens = Number(
        item.opens ?? item.total_opens ?? item.open_count ?? item.opened ?? item.opened_count ?? 0,
      );
  ...
  const hasValidData = useMemo(() => {
    ...
    return timelineData.some((item) => item.sent > 0 || item.opens > 0 || item.replies > 0);
  }, [timelineData]);
  ...
  const senderPieData = useMemo(() => {
    ...
    return items.map((stat) => ({
      name:
        stat.name ||
        (stat.type === 'gmail' ? 'Gmail' : stat.type === 'outlook' ? 'Outlook' : 'Direct SMTP'),
