...
const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        ...
        const repliedItem = payload.find(p => p.name.includes("Replied") || p.name.includes("Replies") || p.dataKey === "replies");
        ...
        return (
            <div className="bg-white/95 backdrop-blur-2xl p-5 rounded-[24px] shadow-2xl border border-white ring-1 ring-slate-900/5 min-w-[200px] animate-in fade-in zoom-in duration-200">
                ...
                {sentValue > 0 && repliedValue > 0 && (
                    <div className="mt-5 pt-4 border-t border-slate-100/80 space-y-2">
                        <div className="flex items-center justify-between gap-4">
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Reply Rate</span>
