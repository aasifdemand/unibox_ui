  ...
  const insights = [
    {
      ...
      value:
        bounceRate <= 3
          ? t('analytics.value_excellent')
          : bounceRate <= 5
            ? t('analytics.value_good')
            : t('analytics.value_at_risk'),
    ...
    },
    {
      ...
      value:
        openRate >= 25
          ? t('analytics.value_high')
          : openRate >= 15
            ? t('analytics.value_average')
            : t('analytics.value_low'),
      status: openRate >= 25 ? 'success' : openRate >= 15 ? 'warning' : 'danger',
      icon: <Zap className="w-5 h-5" />,
      recommendation:
        openRate < 20 ? t('analytics.rec_engagement_low') : t('analytics.rec_engagement_high'),
    },
    {
      id: 'conversion',
      ...
      value:
        replyRate >= 3
          ? t('analytics.value_elite')
          : replyRate >= 1
            ? t('analytics.value_solid')
            : t('analytics.value_needs_work'),
      status: replyRate >= 3 ? 'success' : replyRate >= 1 ? 'warning' : 'danger',
      icon: <TrendingUp className="w-5 h-5" />,
      recommendation:
        replyRate < 1.5 ? t('analytics.rec_conversion_low') : t('analytics.rec_conversion_high'),
    },
  ];
    ...
    <div className="premium-card p-8 h-full flex flex-col relative overflow-hidden group">
      ...
      <div className="grid grid-cols-1 gap-5 relative z-10">
        {insights.map((insight) => (
          <div
            key={insight.id}
            className={`p-5 rounded-lg border transition-all duration-300 hover:shadow-sm hover:-translate-y-1 ${getStatusStyles(insight.status)}`}
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                <div
                  className={`p-2 rounded-md bg-white/80  shadow-sm border border-transparent`}
                >
                  {insight.icon}
                </div>
                <div>
