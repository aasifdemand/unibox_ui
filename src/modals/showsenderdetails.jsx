...
const ShowSenderDetails = ({
  ...
  const getProviderIcon = () => {
    switch (sender.type) {
      case 'gmail':
        return <Gmail className="w-10 h-10" />;
      case 'outlook':
        return <MicrosoftOutlook className="w-10 h-10" />;
      default:
        return <Smtp className="w-10 h-10" />;
    }
  };
    ...
    <Modal isOpen={!!sender} onClose={onClose} maxWidth="max-w-3xl" closeOnBackdrop={true}>
        ...
        <div className="relative flex items-center gap-6">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <div className="px-2 py-0.5 rounded-lg bg-white/20  border border-white/30 text-[9px] font-black uppercase tracking-widest text-white">
                {sender.type}
              </div>
            </div>
      ...
      <div className="p-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {/* Status Card */}
          <div
            className={`p-5 rounded-lg border-2 ${status.bg} ${status.border} flex flex-col items-center text-center`}
          >
            <div
              className={`w-12 h-12 rounded-lg bg-white border ${status.border} flex items-center justify-center mb-4 text-slate-800`}
            >
              {status.icon}
            </div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">
              {t('modals.details.sender_info.health')}
            </p>
          ...
          <div className="p-5 rounded-lg border-2 bg-slate-50 border-slate-100 flex flex-col items-center text-center">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">
              {t('modals.details.sender_info.setup_type')}
            </p>
            <h4 className="text-sm font-extrabold text-slate-800 uppercase">
              {sender.type === 'smtp'
                ? t('modals.details.sender_info.direct')
                : t('modals.details.sender_info.oauth')}
            </h4>
          </div>
            ...
            <div className="premium-card p-6 bg-slate-50 border-none shadow-none grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
              <div>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">
                  {t('modals.details.sender_info.host')}
                </p>
                <p className="text-sm font-bold text-slate-700">{getSmtpHost()}</p>
              </div>
              <div>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">
                  {t('modals.details.sender_info.port_security')}
                </p>
                <p className="text-sm font-bold text-slate-700">
                  {getSmtpPort()} (
                  {sender.type === 'smtp' ? (sender.smtpSecure ? 'SSL/TLS' : 'StartTLS') : 'OAuth2'}
                  )
                </p>
              </div>
              ...
              {sender.type === 'smtp' && (
                <div className="col-span-1 md:col-span-2">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">
                    {t('modals.details.sender_info.user')}
                  </p>
                  <p className="text-sm font-bold text-slate-700">
                    {sender.smtpUsername || sender.smtpUser || sender.email}
                  </p>
                </div>
              )}
          ...
          {(sender.type === 'smtp' || sender.imapHost) && (
            <div>
              ...
              <div className="premium-card p-6 bg-slate-50 border-none shadow-none grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">
                    {t('modals.details.sender_info.imap_host')}
                  </p>
                  <p className="text-sm font-bold text-slate-700">
                    {sender.imapHost || t('common.default_auto')}
                  </p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">
                    {t('modals.details.sender_info.imap_port_security')}
                  </p>
                  <p className="text-sm font-bold text-slate-700">
                    {sender.imapPort || '993'} (
                    {sender.imapSecure !== false ? 'SSL/TLS' : t('common.non_secure')})
                  </p>
                </div>
                ...
                {sender.type === 'smtp' && (
                  <div className="col-span-1 md:col-span-2">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">
                      {t('modals.details.sender_info.imap_user')}
                    </p>
                    <p className="text-sm font-bold text-slate-700">
                      {sender.imapUsername ||
                        sender.imapUser ||
        ...
        <div className="mt-12 pt-8 border-t border-slate-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={onClose}
              className="px-8 py-3 rounded-lg text-[10px] font-extrabold uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-all"
            >
              {t('common.close')}
            </button>
            <Button
