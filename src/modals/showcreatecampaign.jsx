...
const ShowCreateCampaign = ({ showModal, setShowModal }) => {
  ...
  const steps = [
    {
      number: 1,
      title: t('campaigns.step_import_leads'),
      description: t('campaigns.step_import_leads_desc'),
    },
    {
      number: 2,
    ...
    {
      number: 4,
      title: t('campaigns.step_final_review'),
      description: t('campaigns.step_final_review_desc'),
    },
  ];
  ...
  const nextStep = async (e) => {
    ...
    switch (currentStep) {
      ...
      case 2: {
        ...
        if (!hasContent) {
          toast.error(
            t('campaigns.error_add_content', {
              mode: editorMode === 'html' ? 'HTML' : t('campaigns.plain_text'),
            }),
          );
          return;
        }
  ...
  return (
    <Modal isOpen={showModal} onClose={handleClose} maxWidth="max-w-6xl" closeOnBackdrop={true}>
      <div className="relative bg-white flex flex-col h-full max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar px-8 py-8 md:px-12 md:py-10">
          <div className="max-w-5xl mx-auto space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200/60 shadow-sm shadow-slate-200/40 p-10 relative overflow-hidden ">
              <form
          ...
          <div className="flex items-center gap-3">
            {currentStep > 1 && (
              <button
                ...
                className="px-6 py-3 bg-white border border-slate-200 rounded-lg text-[10px] font-extrabold uppercase tracking-widest text-slate-600 hover:border-slate-300 transition-all active:scale-95 flex items-center gap-2 shadow-sm"
              >
            ...
            {currentStep < steps.length ? (
              ...
              >
                ...
                {createCampaign.isPending
                  ? t('campaigns.creating_campaign')
                  : t('campaigns.create_campaign_btn')}
              </button>
            )}
          </div>
        </div>
